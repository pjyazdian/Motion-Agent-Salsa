...
def train(model, train_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            caption, motion = batch['caption'], batch['motion']  # assumes your custom DataLoader outputs this
        ...
        if (epoch + 1) % args.save_every == 0:
            save_path = os.path.join(args.save_dir, f"motionllm_epoch{epoch+1}.pth")
...
def main():
    ...
    args.wandb_run_name = "First trial"
    if args.use_wandb:
    ...
    from your_data_module import YourCustomDataset
    train_dataset = YourCustomDataset(args)
